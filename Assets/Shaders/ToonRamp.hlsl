...
in float ToonRampOffset, in float ToonRampHalftoneOffset, out float3 ToonRampOutput, out float3 Direction)
{
	...
	// set the shader graph node previews
		...
		half toonRamp = smoothstep(ToonRampOffset, ToonRampOffset+ ToonRampSmoothness, d ); //превращает в ноль и один и сохраняет интервал между ними
		if (d > ToonRampOffset && d < ToonRampHalftoneOffset)
		{
			toonRamp = 0.7;
		}
